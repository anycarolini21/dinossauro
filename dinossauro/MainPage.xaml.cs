...
public partial class MainPage : ContentPage
{
	...
	void CorrigeTamanhoCenario(double w, double h)
	{
		...
		foreach (var A in floresta2.Children)
			(A as Image).WidthRequest = w;
		foreach (var A in chao3.Children)
			(A as Image).WidthRequest = w;

		HSLayer1.WidthRequest = w;
		floresta2.WidthRequest = w;
		chao3.WidthRequest = w;
	}
	...
	void GerenciaCenarios()
	{
		...
		GerenciaCenario(floresta2);
		GerenciaCenario(chao3);

	}
	void MoveCenario()
	{
		HSLayer1.TranslationX -= velocidade2;
		floresta2.TranslationX -= velocidade1;
		chao3.TranslationX -= velocidade;
